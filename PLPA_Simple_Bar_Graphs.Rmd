...
## SECTION 8: Bar Plot and P-value table

*Purpose*

- # Loop over each major pathway to draw its bar plot and print the module (pathway/categoy)‐level p‐values.

```{r}
 ---- 8. Generate Bar Plots & Print Significance ----

# List of pathway data objects
pathway_data <- list(
...
# Corresponding significance tables
sig_tables <- list(
...
# Loop pathway, plot bar chart, then print p-values
purrr::walk(names(pathway_data), function(name) {
  plt <- plot_bar(pathway_data[[name]], name)   # draw the bar plot
  print(plt)                                    # display it
  cat("\nModule‐level p‐values for", name, ":\n") 
  print(sig_tables[[name]])                     # show the stats table
})
...
- Build a named list where each entry holds the mean‐abundance matrices (raw + normalized) for one major pathway by calling `calculate_pathway_data()` on your categories.

2. `sig_tables <- list(...)`  

- Create a matching named list of significance tables (`data.frame`s of p-values) for each pathway.

3. `purrr::walk(names(pathway_data), function(name) { … })`  

- Loop over each pathway name, running the anonymous function on every element of `pathway_data`.

4. `plt <- plot_bar(pathway_data[[name]], name)`  

- Inside the loop: generate the faceted bar chart for the current pathway’s raw abundance matrix.

5. `print(plt)`  

- Display the bar chart in the R plotting window.

6. `cat("\nModule-level p-values for", name, ":\n")`  

- Print a header announcing which pathway’s module p-values are coming next.

7. `print(sig_tables[[name]])`  

- Print the data frame of module-level significance results for that pathway. 

*Key Packages*

- purrr: for `walk()`, to loop without returning a value.

- ggplot2: for plotting inside `plot_bar()`.

- dplyr, tidyr, tibble: used earlier to shape data passed into these functions.